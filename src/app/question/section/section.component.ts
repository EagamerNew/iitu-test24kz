...
export class SectionComponent implements OnInit {
  ...
  onSave() {
    ...
    if (newCategory.id) {
      this.firestore.doc('sections/' + newCategory.id).update({
        'name': newCategory.name,
        'categoryId': newCategory.categoryId
      })
      this.questionSectionForm.reset();
      this.getSections();
      this.openSnackBar('Раздел успешно изменено !', '');
    } else {
      this.firestore.collection('sections').add({
        'name': newCategory.name,
        'categoryId': newCategory.categoryId
      })
      this.questionSectionForm.reset();
      this.getSections();
      this.openSnackBar('Раздел успешно сохранено !', ''); 
    }
  }
  ...
  delete(section: QuestionSection) {
    ...
    this.firestore.doc('sections/'+section.id).delete()
    this.getSections();
    this.openSnackBar('Раздел успешно удалено !', '');
  }
